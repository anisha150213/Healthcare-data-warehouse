...
class ClinicalDataSystem:
    ...
    def add_patient_or_visit(self, values: dict[str, Any]):
    ...
    def count_visits(self, visit_date: str):
    ...
    def identify_eligible_patients(self):
        return sorted(
            patient.patient_id
    ...
    def generate_key_statistics(self, output_folder: str):
    ...
    def export_dict_to_csv(self, filename: str, rows: dict[str, object], output_folder: str):
        output_path = Path(output_folder)
        output_path.mkdir(parents=True, exist_ok=True)
    ...
    def export_workload_to_csv(self, output_folder: str):
        output_path = Path(output_folder)
        output_path.mkdir(parents=True, exist_ok=True)
    ...
    def _random_unique_id(self, prefix: str, existing: dict[str, Any]):
        while True:
    ...
    def _required_text(self, values: dict[str, Any], key: str):
        value = str(values.get(key, "")).strip()
        if not value:
            raise ValueError(f"{key} is required.")
        return value

    def _required_int(self, values: dict[str, Any], key: str):
        return int(float(self._required_text(values, key)))

    def _required_float(self, values: dict[str, Any], key: str):
        return float(self._required_text(values, key))

    def _optional_float(self, value: object):
        text = str(value or "").strip()
        return None if not text else float(text)

    def _write_gender_plot(self, output_path: Path, gender_counts: dict[str, int]):
        try:
